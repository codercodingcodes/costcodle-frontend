...
function WinScreen({guessCnt,gameData,toggle,users}:{guessCnt:number,gameData:GameInfo,toggle:()=>void,users:UserData[]}) {
    return (
        <div className="w-full h-full text-center bg-gray-100 fixed">
            ...
            <p className={"absolute font-costco text-red-600 md:text-blue-600 text-2xl md:text-4xl top-1/3 md:top-2/3 left-1/2 -translate-x-1/2 w-full"}>Congrats! You got today's item in {guessCnt} tries!</p>
            <p className={"absolute font-costco text-xl md:text-2xl top-1/2 md:top-4/5 left-1/2 -translate-x-1/2 w-full"}>Next game in : {24-Math.floor(gameData.time / 3600)} hrs</p>
